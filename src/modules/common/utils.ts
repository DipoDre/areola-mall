...
let envJwtSecret: Secret = AUTH_SECRET as Secret;
...
export async function Authenticate(
	...
	try {
		...
		// @ts-ignore
		const user: IAuthUser = jwt.verify(token, process.env.AUTH_SECRET);
		req.user = user;
		return next();
	} catch (e) {
...
export function errorHandler(
	...
	try {
		...
		let msg = error.message || "Exception 500! Operation failed.";
